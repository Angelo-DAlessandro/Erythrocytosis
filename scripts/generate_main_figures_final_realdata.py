...
clinical_cols=[c for c in clinical_cols if c!='Gender']
protein_cols=list(df.columns[26:1488])
met_cols=list(df.columns[1488:])
order=['01-Ctrl','02-Family','03-VHL','03-EGLN2','04-EPAS1','05-EPOR','06-HAH','07-NDD','08-NDD/PDE4']
labels_short={'01-Ctrl':'Ctrl','02-Family':'Family','03-VHL':'VHL','03-EGLN2':'EGLN2','04-EPAS1':'EPAS1','05-EPOR':'EPOR','06-HAH':'HAH','07-NDD':'NDD','08-NDD/PDE4':'NDD/PDE4'}
palette={'01-Ctrl':'#6e7f80','02-Family':'#F2A93B','03-VHL':'#6A3D9A','03-EGLN2':'#4C78A8','04-EPAS1':'#1F9EB3','05-EPOR':'#2DBA61','06-HAH':'#29A889','07-NDD':'#9C755F','08-NDD/PDE4':'#C8D800'}
def_groups=['03-VHL','03-EGLN2','04-EPAS1','05-EPOR','06-HAH']
...
def panel_label(ax, s, x=-0.10, y=1.06):
    ax.text(x, y, s, transform=ax.transAxes, fontweight='bold', fontsize=14, ha='left', va='top')
...
def nice_feature_name(s, is_met=False):
    ...
    clinical_map={
        ...
        'Ferritina (mg/dL)':'Ferritin','Transferrin (mg/dL)':'Transferrin','Uric A (mg/dL)':'Uric acid',
        ...
        'ALT (U/L)':'ALT','AST (U/L)':'AST','eGFR (ml/min)':'eGFR','Age':'Age'
    }
...
def heatmap_square(ax, X, features, y, title, is_met=False, panel=''):
    ...
    ax.set_yticks(np.arange(nrow)); ax.set_yticklabels([nice_feature_name(f,is_met) for f in feats_order], fontsize=4.55)
    ax.set_xticks([])
    ax.set_title(title, fontweight='bold', fontsize=9.5, pad=19)
    # group top bars
    ...
    for g in order:
        ...
        if n:
            ...
            ax.text(start+n/2-0.5,ytext,f'{labels_short[g]}\n(n={n})',ha='center',va='bottom',rotation=0,fontsize=4.2,clip_on=False)
            start+=n
    ...
    for r,f in enumerate(feats_order):
        ...
        if st: ax.text(ncol+0.20, r, st, va='center', ha='left', fontsize=6.4, fontweight='bold')
    if panel:
        ax.text(-0.13, 1.09, panel, transform=ax.transAxes, fontweight='bold', fontsize=14, ha='left', va='top')
    for sp in ax.spines.values(): sp.set_linewidth(0.6); sp.set_color('0.4')
    return im

# ---------------- FIGURE 1 ----------------
fig=plt.figure(figsize=(16.0,10.2))
gs=GridSpec(2,3,figure=fig,height_ratios=[0.85,1.25],width_ratios=[1.05,1,1.05],hspace=0.10,wspace=0.36)
# A
axA=fig.add_subplot(gs[0,0]); axA.axis('off'); panel_label(axA,'A',x=-0.06,y=1.02)
axA.set_title('Cohort and data layers',fontweight='bold',fontsize=10,pad=4)
...
for label,n,col,y0 in [('Control',7,'#6e7f80',0.72),('Family',4,'#F2A93B',0.50),('Familial erythrocytosis',52,'#D62728',0.28)]:
    ...
    axA.text(0.245,y0+0.065,f'{label}\n(n={n})',color='white',ha='center',va='center',fontsize=8.5,fontweight='bold')
axA.add_patch(FancyArrowPatch((0.48,0.57),(0.58,0.57),arrowstyle='-|>',mutation_scale=16,lw=1.4,color='0.35'))
subgroups=['03-VHL','03-EGLN2','04-EPAS1','05-EPOR','06-HAH','07-NDD','08-NDD/PDE4']
...
for g,(x,y0) in zip(subgroups,coords):
    ...
    axA.text(x+0.08,y0+0.055,f'{labels_short[g]}\n(n={n})',color='white' if g!='08-NDD/PDE4' else 'black',ha='center',va='center',fontsize=6.9,fontweight='bold')
for i,(lab,col) in enumerate([('Clinical','#9ecae1'),('Proteome','#fdae6b'),('Metabolome','#a1d99b')]):
    ...
    axA.text(xx+0.085,0.078,lab,ha='center',va='center',fontsize=7,fontweight='bold')
...
axA.text(0.03,-0.03,f'Final analytic cohort: n={len(D)} after excluding 2 multivariate outliers',fontsize=7.6,ha='left')
...
for g in ['Control','Family','Erythrocytosis']:
    ...
    axB.scatter(coords[m,0],coords[m,1],s=36,color=col,edgecolor='white',lw=0.45,alpha=0.95,zorder=2)
    add_ellipse(axB,coords[m,0],coords[m,1],col,alpha=0.14)
    axB.text(coords[m,0].mean(),coords[m,1].mean(),f'{g}\n(n={m.sum()})',ha='center',va='center',fontsize=7.1,fontweight='bold',bbox=dict(fc='white',ec=col,lw=0.8,alpha=0.9,boxstyle='round,pad=0.18'))
...
axB.set_title('PLS-DA: Controls, Family, and All Erythrocytosis',fontweight='bold',fontsize=9)
...
for g in def_groups:
    ...
    axC.scatter(coords2[m,0],coords2[m,1],s=36,color=col,edgecolor='white',lw=0.45,alpha=0.95,zorder=2)
...
axC.set_title('PLS-DA: Defined erythrocytosis subgroups',fontweight='bold',fontsize=9)
...
cax=fig.add_axes([0.935,0.16,0.012,0.38]); cb=fig.colorbar(im,cax=cax); cb.set_label('Autoscaled abundance\n(z-score)',fontsize=8); cb.ax.tick_params(labelsize=7)
legax=fig.add_axes([0.922,0.055,0.07,0.09]); legax.axis('off')
legax.add_patch(Rectangle((0,0),1,1,fc='white',ec='0.65',lw=0.7))
legax.text(0.5,0.82,'Significance\n(p-value)',ha='center',va='top',fontsize=6.5,fontweight='bold')
for i,(st,txt) in enumerate([('*','p < 0.05'),('**','p < 0.01'),('***','p < 0.001'),('****','p < 0.0001')]):
    legax.text(0.18,0.50-i*0.14,st,ha='right',fontsize=6.5,fontweight='bold'); legax.text(0.25,0.50-i*0.14,txt,ha='left',fontsize=6.2)
fig.suptitle('Figure 1. Integrated clinical, metabolomic and proteomic architecture of familial erythrocytosis',fontsize=14,fontweight='bold',y=0.985)
fig.savefig(os.path.join(OUT,'Main_Figure_1_final_realdata.svg'),bbox_inches='tight')
plt.close(fig)
...
def module_heat(ax,M,members,title):
    ...
    ax.set_xticks(range(len(def_groups))); ax.set_xticklabels(def_labels,rotation=35,ha='right',fontsize=6.8)
    yl=[]
    ...
    ax.set_yticks(range(len(mat.index))); ax.set_yticklabels(yl,fontsize=7)
...
gs=GridSpec(3,5,figure=fig,height_ratios=[1.25,1.50,1.0],width_ratios=[1.85,0.18,1.25,1.25,1.25],hspace=0.58,wspace=0.48)
# A/B stacked in col 0 via subgrid
leftgs=GridSpecFromSubplotSpec(2,1,subplot_spec=gs[0,0],hspace=0.65)
axA=fig.add_subplot(leftgs[0]); panel_label(axA,'A',x=-0.55,y=1.18)
imA=module_heat(axA,Mmet,met_members,'Metabolite pathway programs')
cb=fig.colorbar(imA,ax=axA,orientation='horizontal',fraction=0.055,pad=0.22); cb.set_label('Median pathway score (z)',fontsize=6); cb.ax.tick_params(labelsize=6)
axB=fig.add_subplot(leftgs[1]); panel_label(axB,'B',x=-0.55,y=1.18)
imB=module_heat(axB,Mprot,prot_members,'Proteomic pathway programs')
cb=fig.colorbar(imB,ax=axB,orientation='horizontal',fraction=0.055,pad=0.22); cb.set_label('Median pathway score (z)',fontsize=6); cb.ax.tick_params(labelsize=6)
...
def volcano_cov(ax,cov_col,title,letter):
    ...
    for k,(_,r) in enumerate(lab.iterrows()):
        ...
        ax.annotate(labname,xy=(r['rho'],r['mlogp']),xytext=(r['rho']+dx,r['mlogp']+dy),fontsize=5.2,ha='left' if side>0 else 'right',va='bottom',color='#7f1d1d' if side>0 else '#234b83',arrowprops=dict(arrowstyle='-',lw=0.35,color='0.5',alpha=0.6),clip_on=False)
    ax.set_title(title,fontweight='bold',fontsize=9)
    ax.set_xlabel('Spearman rho',fontsize=7); ax.set_ylabel('-log10 p',fontsize=7)
    ax.set_xlim(-1.02,1.02); ax.tick_params(labelsize=7)
    ax.text(0.98,0.92,f'n={sig.sum()} covariates',ha='right',va='top',transform=ax.transAxes,fontsize=6,color='0.35')
for i,(cov,title,letter) in enumerate([('Emogas pHv','pH-linked covariates','C'),('Glucose (mg/dL)','Glucose-linked covariates','D'),('vLac (mmol/L)','Lactate-linked covariates','E')]):
    ax=fig.add_subplot(gs[0,i+2]); volcano_cov(ax,cov,title,letter)
# Panel F network across full row
axF=fig.add_subplot(gs[1,:]); panel_label(axF,'F',x=-0.025,y=1.05); axF.axis('off')
axF.set_title('Hive-style map of top clinical-omics correlations',fontweight='bold',fontsize=11,pad=2)
clin_list=['IND BIL (mg/dL)','TOT Bil (mg/dL)','ALT (U/L)','Uric A (mg/dL)','Transferrin (mg/dL)','Ferritina (mg/dL)','vpC02 (mmHg)','vLac (mmol/L)','Emogas pHv','TG','LDL','HDL','MPV (fL)','PLT (/microL)','WBC (/microL)','Neu (/microL)','Glucose (mg/dL)','Age']
...
axF.text(-1,1.05,'Metabolites',ha='center',fontsize=10,fontweight='bold')
axF.text(0,1.05,'Clinical',ha='center',fontsize=10,fontweight='bold')
axF.text(1,1.05,'Proteins',ha='center',fontsize=10,fontweight='bold')
for key,(x,y) in pos.items():
    ...
    if kind=='met': axF.text(x-0.045,y,label,ha='right',va='center',fontsize=5.7)
    elif kind=='prot': axF.text(x+0.045,y,label,ha='left',va='center',fontsize=5.7)
    else: axF.text(x,y,label,ha='center',va='center',fontsize=5.0,color='0.25',zorder=4)
for i,(m,c) in enumerate(list(mod_colors.items())[:6]): axF.text(-1.55,0.72-i*0.18,m,color=c,fontsize=7,fontweight='bold',ha='left')
for i,(m,c) in enumerate(list(mod_colors.items())[6:]): axF.text(1.30,0.72-i*0.16,m,color=c,fontsize=7,fontweight='bold',ha='left')
axF.text(0,-1.04,'Nodes represent features within pathway modules with strong clinical Spearman correlations; edge color reflects correlation direction.',ha='center',fontsize=7,color='0.4')
axF.set_xlim(-1.65,1.65); axF.set_ylim(-1.12,1.15)
# G-K scatter
pairs=[('Emogas pHv','MYH10','Prot'),('vLac (mmol/L)','Biliverdin','Met'),('Transferrin (mg/dL)','CCT3','Prot'),('TOT Bil (mg/dL)','CA2','Prot'),('Ferritina (mg/dL)','ATP','Met')]
for k,(xv,yv,kind) in enumerate(pairs):
    ax=fig.add_subplot(gs[2,k]); panel_label(ax,chr(ord('G')+k),x=-0.18,y=1.10)
    ...
    gg=groups[def_mask]
    for g in def_groups:
        m=gg==g; ax.scatter(x[m],y[m],s=25,color=palette[g],alpha=0.9,edgecolor='white',lw=0.4)
    ...
    ax.set_title(f'{nice_feature_name(xv)} vs {nice_feature_name(yv,kind=="Met")}',fontweight='bold',fontsize=9)
    ax.set_xlabel(nice_feature_name(xv),fontsize=7); ax.set_ylabel(nice_feature_name(yv,kind=='Met'),fontsize=7)
    ax.text(0.03,0.96,f'{kind} • ρ={r:.2f}; p={p:.1e}',transform=ax.transAxes,va='top',ha='left',fontsize=5.6,bbox=dict(fc='white',ec='none',alpha=0.72))
...
fig.suptitle('Figure 2. Pathway-level programs and clinical-omics coupling across defined erythrocytosis',fontsize=14,fontweight='bold',y=0.985)
fig.savefig(os.path.join(OUT,'Main_Figure_2_final_realdata.svg'),bbox_inches='tight')
plt.close(fig)

# ---------------- FIGURE 3 ----------------
# classifier panels and NDD mapping
mask_defined=np.isin(groups,def_groups)
mask_unknown=np.isin(groups,['07-NDD','08-NDD/PDE4'])
y_def=np.array([labels_short[g] for g in groups[mask_defined]])
classes=def_labels

def loocv_conf(X, y, k=20):
    # limit k to n_features and train size; LR with balanced class weights
    nfeat=min(k, X.shape[1])
    preds=[]; truth=[]
    loo=LeaveOneOut()
    for tr,te in loo.split(X,y):
        # select k based on training only
        kk=min(nfeat, X.shape[1], len(tr)-1)
        pipe=Pipeline([('sel',SelectKBest(f_classif,k=kk)),('clf',LogisticRegression(max_iter=5000,class_weight='balanced',C=1.0))])
        pipe.fit(X.iloc[tr],y[tr]); preds.append(pipe.predict(X.iloc[te])[0]); truth.append(y[te][0])
    cm=confusion_matrix(truth,preds,labels=classes); acc=accuracy_score(truth,preds)
    rec=np.diag(cm)/np.maximum(cm.sum(axis=1),1)
    return cm,acc,rec
models=[('Clinical',Xclin.loc[mask_defined],min(12,Xclin.shape[1])),('Metabolome',Xmet.loc[mask_defined],25),('Proteome',Xprot.loc[mask_defined],25),('Combined',Xall.loc[mask_defined],35)]
clf_results=[(name,)+loocv_conf(X,y_def,k) for name,X,k in models]
# consensus probabilities for unknowns using LR pipelines trained on defined
prob_list=[]
unk_ids=D.loc[mask_unknown,'Sample_ID'].astype(str).values
for name,X,k in models:
    kk=min(k,X.shape[1],X.shape[0]-1)
    pipe=Pipeline([('sel',SelectKBest(f_classif,k=kk)),('clf',LogisticRegression(max_iter=5000,class_weight='balanced',C=1.0))])
    pipe.fit(X,y_def)
    Xu={'Clinical':Xclin,'Metabolome':Xmet,'Proteome':Xprot,'Combined':Xall}[name].loc[mask_unknown]
    p=pipe.predict_proba(Xu)
    dfp=pd.DataFrame(p,columns=pipe.named_steps['clf'].classes_,index=unk_ids)
    prob_list.append(dfp.reindex(columns=classes).fillna(0))
prob_cons=sum(prob_list)/len(prob_list)
# RF proximity embedding based on combined defined+unknown
Xmap=Xall.loc[mask_defined|mask_unknown]
y_map=np.array([labels_short[g] for g in groups[mask_defined|mask_unknown]])
...
# binary unknown-vs-defined RF importance
Xbin=Xall.loc[mask_defined|mask_unknown]
ybin=np.where(mask_unknown[mask_defined|mask_unknown],'NDD/NDD-PDE4','Defined')
...
gs=GridSpec(3,4,figure=fig,height_ratios=[1.0,1.12,1.30],hspace=0.64,wspace=0.48)
...
for i,(name,cm,acc,rec) in enumerate(clf_results):
    ...
    for r in range(cm.shape[0]):
        for c in range(cm.shape[1]): ax.text(c,r,str(cm[r,c]),ha='center',va='center',fontsize=8,color='white' if cm[r,c]>cm.max()/2 else 'black')
    ax.set_xlabel('Predicted'); ax.set_ylabel('True')
    ax.set_title(f'{name}\naccuracy={acc:.2f}',fontweight='bold',fontsize=9)
    ax.text(0.5,-0.38,'Recall: '+' | '.join([f'{cl} {rv:.2f}' for cl,rv in zip(classes,rec)]),transform=ax.transAxes,ha='center',va='top',fontsize=5.6)
...
axE.set_yticks(range(len(prob_cons.index))); axE.set_yticklabels(prob_cons.index,fontsize=7)
axE.set_xticks(range(len(classes))); axE.set_xticklabels(classes,fontsize=8)
for r in range(prob_cons.shape[0]):
    for c in range(prob_cons.shape[1]):
        val=prob_cons.values[r,c]
        axE.text(c,r,f'{val:.2f}',ha='center',va='center',fontsize=7,color='white' if val<0.25 or val>0.65 else 'black')
axE.set_title('NDD/NDD-PDE4 consensus neighborhood probabilities',fontweight='bold',fontsize=10)
...
# ensure order
for lab in ['VHL','EGLN2','EPAS1','EPOR','HAH','NDD','NDD/PDE4']:
    if lab in y_map:
        m=y_map==lab; col=map_pal.get(lab,'#999999')
        axF.scatter(emb[m,0],emb[m,1],s=32,color=col,edgecolor='white',lw=0.45,alpha=0.95,label=lab,zorder=2)
        add_ellipse(axF,emb[m,0],emb[m,1],col,alpha=0.12)
        axF.text(emb[m,0].mean(),emb[m,1].mean(),lab,ha='center',va='center',fontsize=7.2,fontweight='bold',bbox=dict(fc='white',ec=col,lw=0.8,alpha=0.88,boxstyle='round,pad=0.15'))
axF.set_title('Random-forest proximity: undefined cases map to multiple neighborhoods',fontweight='bold',fontsize=10)
axF.set_xlabel('Proximity PC1'); axF.set_ylabel('Proximity PC2')
axF.legend(loc='upper right',fontsize=7,frameon=True,ncol=1,borderpad=0.3,handletextpad=0.3)
...
axG.set_yticks(ypos); axG.set_yticklabels([nice_feature_name(f, f in Xmet.columns) for f in imp.index],fontsize=6.3)
axG.set_xlabel('Random forest importance')
axG.set_title('Top features distinguishing undefined from defined erythrocytosis',fontweight='bold',fontsize=10)
axG.tick_params(axis='x',labelsize=7)
# H heatmap top features, square cells, horizontal top group labels
axH=fig.add_subplot(gs[2,2]); panel_label(axH,'H',x=-0.18,y=1.12)
features=feat_top[:14]
# samples sorted by groups defined + unknown
mask_h=mask_defined|mask_unknown
Xh=Xall.loc[mask_h,features]
yh=groups[mask_h]
sort=[]
for g in def_groups+['07-NDD','08-NDD/PDE4']:
...
axH.set_yticks(range(len(features))); axH.set_yticklabels([nice_feature_name(f, f in Xmet.columns) for f in features],fontsize=5.8)
...
for g in def_groups+['07-NDD','08-NDD/PDE4']:
    ...
    if n:
        ...
        axH.text(start+n/2-0.5,-1.38,labels_short[g],ha='center',va='bottom',fontsize=4.8,clip_on=False)
        start+=n
axH.set_title('NDD-like feature pattern',fontweight='bold',fontsize=10,pad=24)
...
for v in rep_vars[:5]:
    data.append(Xall.loc[~mask_unknown, v].values); positions.append(p); colors.append('#d9d9d9')
    data.append(Xall.loc[mask_unknown, v].values); positions.append(p+0.32); colors.append('#b98b7e')
...
axI.set_xticks([np.mean(positions[i*2:i*2+2]) for i in range(len(ticklabels))]); axI.set_xticklabels(ticklabels,rotation=35,ha='right',fontsize=7)
axI.set_ylabel('Autoscaled value')
axI.set_title('Representative NDD-associated\nvariables',fontweight='bold',fontsize=10,pad=12)
axI.axhline(0,color='0.85',lw=0.7)
# small legend
axI.scatter([],[],s=30,color='#d9d9d9',edgecolor='0.35',label='Defined')
axI.scatter([],[],s=30,color='#b98b7e',edgecolor='0.35',label='NDD/NDD-PDE4')
axI.legend(loc='upper right',fontsize=6,frameon=False)
fig.suptitle('Figure 3. Multi-omics prediction and molecular characterization of undefined erythrocytosis',fontsize=14,fontweight='bold',y=0.985)
fig.savefig(os.path.join(OUT,'Main_Figure_3_final_realdata.svg'),bbox_inches='tight')
plt.close(fig)
